...
在上面的例子里，选 target = 2 也是类似的。
更一般的，假定一个无重复元素已排序数组为 [x_0,...,x_{n-1}]。假定 target 就等于 x_0 （或者等于 x_{n-1}）。
- 在**左闭右闭**的情况下，每次查询其实都是 right 不断向左移，直到最后 right == left == 0 的过程。
  此时 x_0 需要被查找到，所以当然应该用 <=，否则正确答案就漏掉了。
- 在**左闭右开**的情况下，当然每次也是 right 不断左移，但是最后的状态是 right = left + 1 = 1（代入 [0,1,2] 或者 [1,2] 试试就知道了）。
  此时 x_0 当然也是需要被查找到的，但是用 < 就可以了，因为 left 为 0，right 为 1，本来也是会继续往下查找的。

其实再一般化一点，target 不见得必须是数组的最小值或最大值，在数组中间位置也可以：两边都不断地靠近 target，然后看最后的 while 循环怎么处理。
判断用 <= 还是用 < 的核心点其实就是看 left 和 right 重合时（也就是 left == right 时）的表现。注意：在大部分的时候 left 和 right 没有重合，
也就是 left 小于 right，这种时候不管是用 <= 还是用 < 都不会涉及判断 while 循环还要不要进去（肯定要进去的）的问题。
而当 left == right 时，while 循环要不要进去可以这么想：
- 如果是**左闭右闭**的情况，当前的 right 所在的位置还没被搜索过呢，所以当然要进去，所以此时要用 while left <= right，
  从而保证当 left == right 时，还会再执行一次；
- 反之，对于**左闭右开**的情况，当前的 right 所在的位置已经被搜索过了，所以不用进去了，所以此时要用 while left < right，
  使得当 left == right 时就可以结束了。

此外，补充一点如果 n 为奇数，那么两种方式每次的 mid 都恰好是唯一的中间元素；如果 n 为偶数，那么：
- 在**左闭右闭**的情况下，mid 是中间偏前/偏左/偏小的那个元素，比如 [11,22,33,44]，第一次的 mid 为 22；
- 在**左闭右开**的情况下，mid 是中间偏后/偏右/偏大的那个元素，比如 [11,22,33,44]，第一次的 mid 为 33。
```
