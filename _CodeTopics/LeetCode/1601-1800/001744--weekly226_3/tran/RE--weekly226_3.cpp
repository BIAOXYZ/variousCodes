...
public:
    vector<bool> canEat(vector<int>& candiesCount, vector<vector<int>>& queries) {
        ...
        vector<int> prefixSum(1, candiesCount[0]);
...
/*
https://leetcode-cn.com/submissions/detail/182721259/

54 / 62 个通过测试用例
状态：执行出错

执行出错信息：
Line 14: Char 50: runtime error: signed integer overflow: 2147433023 + 55507 cannot be represented in type 'int' (solution.cpp)
SUMMARY: UndefinedBehaviorSanitizer: undefined-behavior prog_joined.cpp:23:50
最后执行的输入：
*/
/*
躲过了一次RE（maxEat定义和初始化那一行），躲不过第二次（`prefixSum.push_back(prefixSum.back() + candiesCount[i]);`）。。。
*/
