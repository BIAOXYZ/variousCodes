...
想一想为什么这两个值一定是相等的？由于插入了间隔符，所有原始字符串中的回文串，对应到新字符串中的那个回文串的长度都是奇数（基本上任何一个讲马拉车算法的攻略，开头都会提到这个事实）。前面`"abababa"`的情形很容易看出来，我们不妨再以下图中第二个字符串`"abaaba"`为例：其变换后为`"|a|b|a|a|b|a|"`，以中间相邻的那对`aa`为中心的回文子串——也就是以最中间的`|`为中心的回文子串——也就是整个新串——其长度为`13`，照样是奇数。

那么我们可以假定**全字符回文子串的左半边（如果这个全字符回文子串是奇数长度，那么不包括中间那个字符）**的长度为`k`。
于是有:
