class Solution:
    def gridIllumination(self, n: int, lamps: List[List[int]], queries: List[List[int]]) -> List[int]:
        ...
        def be_illuminated(coor):
            ...
            x, y = coor[0], coor[1]
            for lamp in lampSet:
                if x == lamp[0] or y == lamp[1] or x - lamp[0] == y - lamp[1] or x - lamp[0] == -(y - lamp[1]):
                    return 1
            return 0
...
"""
https://leetcode-cn.com/submissions/detail/265692199/

41 / 45 个通过测试用例
状态：超出时间限制
"""
"""
前面那个（`deliberate-TLE-but-WA--001001.py3`）错的主要原因是没有考虑反对角线。
"""
