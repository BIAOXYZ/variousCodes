...
class Solution(object):
    def maxFrequency(self, nums, k):
        ...
        for rightBoundary in range(1, length):
            ...
            while low <= high:
                mid = (low + high) / 2
                total = (rightBoundary - mid) * nums[rightBoundary]
                # 这里要防止下 mid 为 0 时一下子减到倒数第一个元素这种意外情况。
                prefixSum_mid_to_rightBountry = prefixSum[rightBoundary-1] - prefixSum[mid-1] if mid > 0 else prefixSum[rightBoundary-1] - 0
...
"""
https://leetcode-cn.com/submissions/detail/201224856/
...
执行用时：2824 ms, 在所有 Python 提交中击败了5.14%的用户
内存消耗：23.4 MB, 在所有 Python 提交中击败了5.40%的用户
"""
