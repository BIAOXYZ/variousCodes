...
|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
||| 1 <br>【`e+` `LC1763 最长的美好子字符串`】<br>【**特点**：暴力枚举；位运算；分治；[:bulb:][`*`]滑动窗口。】 | 2 <br>【`LC2000 反转单词前缀`】<br>【**特点**：字符串；双指针。】<br>【**其他**：`C/C++语法`——`string.find()`、`string::npos` 和 `reverse()`；C语言的 `strchr()`函数；`Python语法`——`str.find()`、`reversed()` 以及字符串切片反转。】 | 3 <br>【`LC1414 和为 K 的最少斐波那契数字数目`】<br>【**特点**：数学+贪心；数学+[:bulb:][`*`]（特殊的）回溯算法。】 | 4 <br>【`LC1725 可以形成最大正方形的矩形数目`】<br>【**特点**：无。】<br>【**其他**：`C/C++语法`——`unordered_map` 如何实现类似Python的 `.keys()` 效果。】<br>【其他2：曾经的周赛题。】 ||
||||||||
