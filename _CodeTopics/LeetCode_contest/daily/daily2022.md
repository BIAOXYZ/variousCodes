...
|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
||| 1 <br>【`e+` `LC1763 最长的美好子字符串`】<br>【**特点**：暴力枚举；位运算；分治；滑动窗口。】 | 2 <br>【`LC2000 反转单词前缀`】<br>【**特点**：字符串；双指针。】<br>【**其他**：`C/C++语法`——`string.find()`、`string::npos` 和 `reverse()`；C语言的 `strchr()`函数；`Python语法`——`str.find()`、`reversed()` 以及字符串切片反转。】 ||||
||||||||
