...
| 9 <br>【`LC1629 按键持续时间最长的键`】<br>【**特点**：模拟。】<br>【其他：曾经的周赛题。】 | 10 <br>【`LC306 累加数`】<br>【**特点**：模拟；回溯算法。】<br>【**其他**：`C/C++语法`——lambda匿名函数（可以先用一行定义匿名函数的名字，然后另起一行开始写）；`vector` 倒数第二个元素 `v[v.size()-2]`，因此最后一个元素除了用 `v.back()` 外也可以用 `v[v.size()-1]`。】 ||||||
||||||||
