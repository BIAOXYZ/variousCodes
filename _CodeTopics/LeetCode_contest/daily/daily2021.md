...
| 30 <br>【[:bulb:][`*`]`LC231 2的幂`】<br>【**特点**：位运算；数学。】<br>【**其他**：[:bulb:][`*`]位运算小技巧：除了`LC338`、`LC1178`和`LC461`里用到的`n &= n - 1`的trick外；又有一个新的trick：`n & (-n)`为 n 的最右边（最低位）的1加上后面的所有0，跟负数的补码表示有关，不详细展开了。】 |||||||
