...
||||| 6 <br>【`LC1720 解码异或后的数组`】<br>【**特点**：位运算。】<br>【**其他**：`C++语法`——`vector`对象取末尾元素的四种方法；`vector`对象仅包含一个元素的（两种）初始化方法。】 | 7 <br>【`LC1486 数组异或操作`】<br>【**特点**：位运算；[:bulb:][`*`]数学（异或运算的性质）。】<br>【**其他**：数学技巧：`∀i ∈ Z`，有 `4i ⊕ (4i+1) ⊕ (4i+2) ⊕ (4i+3) = 0`。】<br>【**其他2**：曾经的周赛题。】 ||
||||||||
