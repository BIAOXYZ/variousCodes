...
|||| 26 <br>【`LC1190 反转每对括号间的子串`】<br>【**特点**：栈。】<br>【**其他**：前面22号到25号连着四天`Hard`。。。最近工作任务又重，直接看一眼就不管了。。。】 | 27 <br>【`LC461 汉明距离`】<br>【**特点**：位运算；数学；[:bulb:][`*`]Brian Kernighan 算法。】<br>【**其他**：`C语法` and `C++语法`——内置的统计某个数二进制形式里1的个数的函数`__builtin_popcount()`。】 |||
||||||||
