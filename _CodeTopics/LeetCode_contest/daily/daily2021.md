...
| 19 <br>【`LC650 只有两个键的键盘`】<br>【**特点**：动态规划；数学（因子分解）。】 | 20 <br>【`m+` `LC673 最长递增子序列的个数`】<br>【**特点**：动态规划（[:bulb:][`*`]序列DP）；贪心+前缀和+二分查找；树状数组；线段树。】<br>【**其他**：经典题 `LC300 最长递增子序列` 的完全增强版。】 | 21 <br>【`LC58 最后一个单词的长度`】<br>【**特点**：字符串。】 |||||
||||||||
