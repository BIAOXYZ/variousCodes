...
| 8 <br>【`LC1137 第 N 个泰波那契数`】<br>【**特点**：动态规划；记忆化搜索；数学（矩阵快速幂）。】 | 9 <br>【[`LC313 超级丑数`]()】<br>【**特点**：堆；动态规划。】 | 10 <br>【`LC413 等差数列划分`】<br>【**特点**：差分 + 计数。】 | 11 <br>【`Hard` [`LC446 等差数列划分 II - 子序列`]()】<br>【**特点**：动态规划 + 哈希表。】<br>【**其他**：`Python语法`——`collections`里的`defaultdict`数据结构，用在这儿（二维dp，第一维是列表，但第二维是字典）挺合适。】 | 12 <br>【`LC516 最长回文子序列`】<br>【**特点**：动态规划。】 |||
||||||||
