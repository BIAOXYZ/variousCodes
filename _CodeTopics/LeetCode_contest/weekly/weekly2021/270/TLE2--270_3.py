    ...
    def getDirections(self, root, startValue, destValue):
        ...
        for i in range(min(len(paths[0]), len(paths[1]))):
            if paths[0][i] == paths[1][i]:
                ...
                break
        pathLeft = paths[0][commonPrefixLen:]
        pathRight = paths[1][commonPrefixLen:]
        
        return 'U'*len(pathLeft) + ''.join(pathRight)
    
"""
https://leetcode-cn.com/submissions/detail/245397037/
